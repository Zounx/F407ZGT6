...
| 方向 | FID | body | 说明 |
|------|-----|------|------|
| PC→设备 | 128 | programInfo[5000] | 保存程序数据，5000 字节，真实长度按 bodyLength 截取 |
| 设备→PC | 129 | result(4B) | 0=失败，1=成功 |
...
- body = `01 00 00 00` = programInfo 前 4 字节（程序 ID=1）
...
> 实际使用时 body 为完整的程序数据（最大 5000 字节），上位机通过 bodyLength 指定真实长度。
