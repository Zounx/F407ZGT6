...
int32_t loopback_udps(uint8_t sn, uint8_t* buf, uint16_t port) {
    ...
    case SOCK_UDP :
        if ((size = getSn_RX_RSR(sn)) > 0) {
            ...
            ret = wiz_recvfrom(sn, buf, size, destip, (uint16_t*)&destport);
            if (ret <= 0) {
#ifdef _LOOPBACK_DEBUG_
            ...
            sentsize = 0;
            while (sentsize != size) {
                ret = wiz_sendto(sn, buf + sentsize, size - sentsize, destip, destport);
                if (ret < 0) {
#ifdef _LOOPBACK_DEBUG_
...
int32_t loopback_udpc(uint8_t sn, uint8_t* buf, uint8_t* destip, uint16_t destport) {
    ...
    switch (getSn_SR(sn)) {
    case SOCK_UDP :
        // wiz_sendto(sn, strtest, strlen(strtest), destip, destport);
        if ((size = getSn_RX_RSR(sn)) > 0) {
            if (size > DATA_BUF_SIZE) {
                size = DATA_BUF_SIZE;
            }
    ...
    case SOCK_CLOSED:
        ...
        // printf("%d:UDP loopback start\r\n",sn);
#endif
        if ((ret = wiz_socket(sn, Sn_MR_UDP, any_port, 0x00)) != sn) {
            return ret;
        }
#ifdef _LOOPBACK_DEBUG_
        printf("%d:Opened, UDP loopback, port [%d]\r\n", sn, any_port);
...
int32_t loopback_tcps(uint8_t sn, uint8_t* buf, uint16_t port) {
    ...
    uint16_t dst_port;
#endif
    wiz_getsockopt(sn, SO_STATUS, &status);
    switch (status) {
    case SOCK_ESTABLISHED :
        wiz_ctlsocket(sn, CS_GET_INTERRUPT, &inter);
        if (inter & Sn_IR_CON) {
#ifdef _LOOPBACK_DEBUG_
            wiz_getsockopt(sn, SO_DESTIP, dst_ip);
            wiz_getsockopt(sn, SO_EXTSTATUS, &ext_status);
            ...
            wiz_getsockopt(sn, SO_DESTPORT, &dst_port);
            printf("Peer Port : %d\r\n", dst_port);
#endif
            arg_tmp8 = Sn_IR_CON;
            wiz_ctlsocket(sn, CS_CLR_INTERRUPT, &arg_tmp8);
        }
        wiz_getsockopt(sn, SO_RECVBUF, &received_size);
        ...
        if (received_size > 0) {
            ...
            // 20231018 taylor
#ifdef _LOOPBACK_DEBUG_
            wiz_getsockopt(sn, SO_DESTIP, destip);
            wiz_getsockopt(sn, SO_DESTPORT, &destport);
            wiz_getsockopt(sn, SO_EXTSTATUS, &sn_status);
...
#endif
            arg_tmp8 = Sn_IR_CON;
            wiz_ctlsocket(sn, CS_CLR_INTERRUPT, &arg_tmp8); // this interrupt should be write the bit cleared to '1'
        }
...
int32_t loopback_udpc(uint8_t sn, uint8_t* buf, uint8_t* destip, uint16_t destport) {
    ...
    case SOCK_UDP :
        ...
        if ((size = getSn_RX_RSR(sn)) > 0) {
            ...
            ret = wiz_recvfrom(sn, buf, size, destip, (uint16_t*)&destport, &addr_len);
            buf[ret] = 0x00;
            printf("recv form[%d.%d.%d.%d][%d]: %s\n", destip[0], destip[1], destip[2], destip[3], destport, buf);
            if (ret <= 0) {
#ifdef _LOOPBACK_DEBUG_
            ...
            sentsize = 0;
            while (sentsize != size) {
                ret = wiz_sendto(sn, buf + sentsize, size - sentsize, destip, destport, addr_len);
                if (ret < 0) {
#ifdef _LOOPBACK_DEBUG_
                    printf("%d: sendto error. %ld\r\n", sn, ret);
#endif
    ...
    case SOCK_CLOSED:
        ...
        switch (loopback_mode) {
        case AS_IPV4:
            wiz_socket(sn, Sn_MR_UDP4, destport, SOCK_IO_NONBLOCK);
            break;
        case AS_IPV6:
            wiz_socket(sn, Sn_MR_UDP6, destport, SOCK_IO_NONBLOCK);
            break;
        case AS_IPDUAL:
            wiz_socket(sn, Sn_MR_UDPD, destport, SOCK_IO_NONBLOCK);
            break;
        }
