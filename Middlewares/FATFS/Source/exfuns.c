...
/* 逻辑磁盘工作区（改为静态数组，不再从 mymalloc 动态分配） */
FATFS fs_pool[FF_VOLUMES];

/* 文件复制共享缓冲（8KB，放在 AXI SRAM） */
static uint8_t g_file_copy_buf[8192];
                                      ...
                                      uint32_t totsize, uint32_t cpdsize, uint8_t fwmode)
{
    ...
    FIL fsrc;           /* 源文件（栈上分配） */
    FIL fdst;           /* 目标文件（栈上分配） */
    uint8_t *fbuf = g_file_copy_buf;  /* 共享 8KB 缓冲 */
    uint8_t curpct = 0;
    ...
    if (res == 0)           /* 两个都打开成功了 */
    {
        ...
        while (res == 0)    /* 开始复制 */
        {
            res = f_read(&fsrc, fbuf, 8192, (UINT *)&br);   /* 源头读出 */
...
uint32_t exfuns_get_folder_size(uint8_t *fdname)
{
    uint8_t res = 0;
    DIR fddir;                      /* 目录（栈上分配） */
    FILINFO finfo;                  /* 文件信息（栈上分配） */
    uint8_t pathname[512 + 1];      /* 目标文件夹路径+文件名 */
                           ...
                           uint32_t *totsize, uint32_t *cpdsize, uint8_t fwmode)
{
    ...
    uint8_t dstpathname[512 + 1];       /* 目标文件夹路径+文件名 */
    uint8_t srcpathname[512 + 1];       /* 源文件夹路径+文件名 */
